...
function runMap(){ 
  ...
  $.getJSON('js/area.json', function(data) {
    ...
    var checkFirst = $(".map div").siblings().hasClass("active");
    if(checkFirst){
      var activeFirst = $(".map .active").attr("id");
      document.getElementById(activeFirst).style.display="block";
      showMap(activeFirst,0,area_maps);
    }

    $selectTarget.change(function(event) {
      var $this = $(this);
      var curTabName = $this.find('option:selected').val();
      ...
      for(var i = 0; i < area_maps.length; i++) {
        ...
        document.getElementById(area_maps[i].id_show).style.display="none";

        if(curTabName == area_maps[i].id_show) {
          if(curTab.hasClass("active")) {
            curTab.removeClass("active");
            document.getElementById(area_maps[i].id_show).style.display="none";
          } else {
            document.getElementById(area_maps[i].id_show).style.display="block";
